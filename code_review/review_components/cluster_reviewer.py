...
class ClusterReviewer:
    ...
    async def review_all_clusters(
        self, clusters: Dict[str, List[Dict[str, Dict]]]
        ...
        for result, cluster_name, file_name in zip(
            results, [t[1] for t in tasks], [t[2] for t in tasks]
        ):
    ...
    async def review_file_async(
        self, file_name: str, file_content: List
    ) -> CodeReviewSummary:
