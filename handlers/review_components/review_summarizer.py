        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            ...
            for file_name, review in file_reviews.items():
                ...
                if len(batch_files) == 10:
                    futures.append(executor.submit(self.__summarize_batch, batch_files))
                    batch_files = []

            if batch_files:
                futures.append(executor.submit(self.__summarize_batch, batch_files))

            batch_summaries = [future.result() for future in futures]

        overall_summary = await self.__summarize_overall(batch_summaries)
        return overall_summary

    def __summarize_batch(self, batch_files: list):
        ...
        try:
            summary = chain_with_history.invoke(
